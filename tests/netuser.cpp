...
void NetUserGetInfoForm(LPCWSTR serverName, LPCWSTR userName) {
  ...
  wprintf(L"\nCalling NetUserGetinfo with Servername=%s Username=%s Level=3\n", serverName, userName);
  nStatus = NetUserGetInfo(serverName, userName, 3, (LPBYTE *) & pBuf);
  // If the call succeeds, print the user information.
  if (nStatus == NERR_Success) {
    if (pBuf != NULL) {
      pBuf3 = (LPUSER_INFO_3) pBuf;
      form_print_usri3(pBuf3);
    }
...
void RESNetQueryDisplayInformationList(LPCWSTR serverName, std::string sep=";") {
