...
std::string
 app_name="rest", app_vers="0.1",
 proxy="", user_agent=app_name+"/"+app_vers, server_url="";

#define my_curl_setopt(curl_opt, opt_param) \
 if ((code=curl_easy_setopt(curl, curl_opt, opt_param)) != CURLE_OK) return false;
...
//  my_curl_setopt(CURLOPT_SSL_VERIFYHOST, 0L);
  ...
  if (proxy != "") my_curl_setopt(CURLOPT_PROXY, proxy.c_str());
...
bool curl_alive(std::string url) {
  ...
  if (verbosity == VERBOSE) std::cout << "Checking access to url " << url << std::endl;
  ...
  if (curl) {
    ...
    /* Check for errors */ 
    if(res != CURLE_OK) {
      if (verbosity == VERBOSE) std::cerr << "curl_easy_perform() failed: " << curl_easy_strerror(res) << std::endl;
      return false;
    }
  } else return false;
...
std::vector<std::string> avail_http_mets={ "GET", "HEAD", "POST", "PUT", "PATCH", "DELETE", "CONNECT", "OPTIONS", "TRACE" };
...
std::string check_http_met(std::string s) {
  ...
  for (auto avail_http_met : avail_http_mets) {
    if (s == avail_http_met) return avail_http_met;
  }
...
std::vector<std::string> split(std::string s, char delim=' ') {
  ...
  while(std::getline(ss, l, delim)) v.push_back(l);
  return v;
}
...
curl --libcurl curl_post.c   -k -X POST -d "login=userBidon&consent=True" https://iyc-tools-rgpdserver-n265-d.epaas.eu.airbus.corp/users
curl --libcurl curl_get.c    -k -X GET                                    https://iyc-tools-rgpdserver-n265-d.epaas.eu.airbus.corp/users/userBidon
curl --libcurl curl_put.c    -k -X PUT  -d  "consent=false"               https://iyc-tools-rgpdserver-n265-d.epaas.eu.airbus.corp/users/userBidon
curl --libcurl curl_delete.c -k -X DELETE                                 https://iyc-tools-rgpdserver-n265-d.epaas.eu.airbus.corp/users/userBidon
...
./rest -r 'POST login=userBidon&consent=True' https://iyc-tools-rgpdserver-n265-d.epaas.eu.airbus.corp/users \
          ...
          -r GET                                 https://iyc-tools-rgpdserver-n265-d.epaas.eu.airbus.corp/users/userBidon \
          -r DELETE                              https://iyc-tools-rgpdserver-n265-d.epaas.eu.airbus.corp/users/userBidon \
          -r GET                                 https://iyc-tools-rgpdserver-n265-d.epaas.eu.airbus.corp/users/userBidon
...
int rest_send(std::string url, std::string request="", bool insecure=false, verbose_mode verbosity=DEFAULT) {
  if (request.length() > 0) {
    ...
    if (verbosity == VERBOSE) std::cout << met << " ";

    if (reqs.size () > 1) {
      if (verbosity == VERBOSE) std::cout << reqs[1] << " " << url << std::endl << std::flush;
      if (met == "POST"  ) return rest_post  (url, reqs[1]);
      if (met == "PUT"   ) return rest_put   (url, reqs[1]);
    } else {
      if (verbosity == VERBOSE) std::cout << url << std::endl << std::flush;
      if (met == "GET")    return rest_get   (url);
      if (met == "DELETE") return rest_delete(url);
    }

    if (verbosity == VERBOSE) std::cout << std::flush;

    return 1;
  } else return rest_get (url);
}
...
int main(int argc, char *argv[]) {
  ...
  if (rest_only) {
    if (insec && verbosity == VERBOSE) std::cout << "insecure rest api call\n";

    for (size_t n=0; n < MIN( http_mets.size(), rest_apis.size() ); n++) {
      auto h=http_mets[n];
      auto r=rest_apis[n];
      std::cout << "R " << r << ", H " << h << std::endl;
      if (verbosity == VERBOSE) std::cout << h << " " << r << std::endl;
      if (rest_send(r, h, insec, verbosity)) return EXIT_FAILURE;
      if (verbosity != SILENT) std::cout << std::endl;
    }
  ...
  for (auto dl_url : dl_urls) {
    if (!curl_download(dl_url, dl_dir, ident, passwd)) return EXIT_FAILURE;
  }

  // Si options -a
  for (auto al_url : al_urls) {
    if (!curl_alive(al_url)) return EXIT_FAILURE;
  }
