...
*/
if (cmd.value === "") {
  cmd.value='msgbox("msg", "titre", 1)';
  //cmd.value='setenvvar("toto", "12")';
  //cmd.value='mywiki()';
  //cmd.value='curl_auth("lalannd2", "ocvdBum12$*4", "https://wiki.space.thales/rest/api/content?limit=2")';
  //cmd.value='curl_auth("lalannd2", "ocvdBum12$*4", "https://wiki.space.thales/rest/api/content/search\?cql=contributor+in+(alkadea,arnones,capous,cavallc,chaumia1,fresnew,guyonnt,kouachb,lalannd2,leleut,moninn,monnete,nottea,thurona,tourel,xsii077,xsii076)+and+space+=+orchestra+and+lastmodified+=+2023-11-07&limit=1000")';
...
function exec_cmd(p_cmd_value) {
    run_cmd.disabled = true;
    cmd_value=encodeToEntities(p_cmd_value);
    ...
    window.webapp_exec(cmd_value).then(result => {
      s=decodeEntities(result.value);//.replace(/(?:\r\n|\r|\n)/g, '<br/>');
