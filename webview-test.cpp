...
constexpr const auto html =
    R"html(
Pass an html file as parameter to the program.
)html";
...
bool swallow_file(LPSTR file_path, std::string &s) {
  std::string fp=file_path;
  return swallow_file(fp, s);
}
...
//  w.set_size(480, 320, WEBVIEW_HINT_NONE);
  ...
  // Run the html page
  std::string s;
  if (lpCmdLine && swallow_file(lpCmdLine, s)) {
    w.set_html(s.c_str());
  } else w.set_html(html);
  w.run();
